...
const PASS = "bg-emerald-500/15 border-emerald-500/40 text-emerald-400";
const BADGE_PASS = "bg-emerald-500 text-black";
    ...
    <div className={`rounded border p-4 ${passed ? PASS : "border-zinc-800 bg-zinc-900/40"}`}>
      <div className="mb-3 flex items-start justify-between gap-2">
        <div>
          ...
          {pValue && (
            <p className="mt-0.5 font-mono text-sm text-amber-400">{pValue}</p>
          )}
        </div>
          ...
          extra={
            <div className="mb-2 grid grid-cols-2 gap-2 rounded bg-zinc-800/50 p-2 text-xs">
              <div>
                ...
                <p className="font-mono font-bold text-amber-400">{mwD["regulated_median_pct"]}%</p>
              </div>
              <div>
          ...
          extra={
            <div className="mb-2 grid grid-cols-2 gap-2 rounded bg-zinc-800/50 p-2 text-xs">
              <div>
                ...
                <p className="font-mono text-amber-400">[{bootR.regulated_ci_95[0]}%, {bootR.regulated_ci_95[1]}%]</p>
              </div>
              <div>
              ...
              {kurtosis.time_series.per_country.map((c) => (
                <div key={c.country} className="flex items-center justify-between text-xs">
                  <span className="text-zinc-400">
                    {c.country}
                    <span className={`ml-1 text-xs ${c.group === "Regulated" ? "text-amber-500" : "text-blue-400"}`}>
                      [{c.group === "Regulated" ? "REG" : "DEREG"}]
                    </span>
                  </span>
                  <span className={`font-mono font-bold ${c.excess_kurtosis > 3 ? "text-amber-400" : "text-zinc-400"}`}>
                    κ = {c.excess_kurtosis}
                  </span>
                </div>
              ))}
